...
var eventBridgeLambdas = []string{"lambda-payment-eventbridge"}
var sqsLambdas = []string{"lambda-payment-sqs"}
...
func buildLambda() error {
	...
	switch t := os.Getenv("TYPE"); t {
	case "sqs":
		lambdas = sqsLambdas
	case "eventbridge":
		lambdas = eventBridgeLambdas
	default:
		lambdas = sqsLambdas
	}
...
func Deploy() error {
	...
	switch t := os.Getenv("TYPE"); t {
	case "sqs":
		template = "lambda-sqs-template.yaml"
	case "eventbridge":
		template = "lambda-eventbridge-template.yaml"
	default:
		template = "lambda-sqs-template.yaml"
	}

	if err := sh.RunV("aws", "cloudformation", "package", "--template-file", path.Join(workingDir, "..", "cloudformation", template), "--output-template-file", path.Join(workingDir, "..", "cloudformation", "lambda-packaged.yaml"), "--s3-bucket", s3bucket); err != nil {
		return err
	}
